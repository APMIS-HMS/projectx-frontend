...
export class InboundRequisitionComponent implements OnInit {
  ...
  onCheckAll(event, i) {
    if (this.existingStockTransfers[i].products.length > 0) {
      if (event.target.checked) {
        this.existingStockTransfers[i].totalQties = 0;
        this.existingStockTransfers[i].totalCost = 0;
        this.existingStockTransfers[i].products.forEach(element2 => {
  ...
  onCheckItem(event, index, index2) {
    if (event.target.checked) {
      this.existingStockTransfers[index].products[index2].isSelected = true;
    }
    else {
      this.existingStockTransfers[index].products[index2].isSelected = false;
    }
    this.existingStockTransfers[index].totalQties = 0;
    this.existingStockTransfers[index].totalCost = 0;
    this.existingStockTransfers[index].products.forEach(element2 => {
      if (element2.isSelected === true) {
        this.existingStockTransfers[index].totalQties += element2.quantityGiven;
        this.existingStockTransfers[index].totalCost += (element2.quantityGiven * element2.costPrice);
      }
    });
  }

  onFocusChangeExistingItemValue(event, index, index2) {
    this.existingStockTransfers[index].products[index2].quantityGiven = event.srcElement.value;
    this.existingStockTransfers[index].totalQties = 0;
    this.existingStockTransfers[index].totalCost = 0;
    this.existingStockTransfers[index].products.forEach(element2 => {
      this.existingStockTransfers[index].totalQties += element2.quantityGiven;
  ...
  onChangeStatue(value, index, index2) {
    this.existingStockTransfers[index].products[index2].status = value.name;
  }
  ...
  onAcceptItem(index) {
    ...
    // this.existingStockTransfers[index].products = JSON.parse(JSON.stringify(this.existingStockTransfers[index].products.filter(x => x.isSelected === true)));
    if (this.existingStockTransfers[index].products.length > 0) {
      this.isButtonDisabled = true;
      this.systemModuleService.on();
      this.inventoryTransferService.createInventoryRequisition(this.existingStockTransfers[index]).then(payload => {
        console.log(payload);
